...
def _back(group_id, page="main"):
    return [InlineKeyboardButton("≪ ʙᴀᴄᴋ", callback_data=f"set_back#{page}#{group_id}")]


def _back_markup(group_id, page):
    return InlineKeyboardMarkup([_back(group_id, page)])
...
def _main_settings_buttons(settings, grp_id):
    ...
    return [
        ...
        [InlineKeyboardButton("🎬 TUTORIAL LINK", callback_data=f"set_page#tutorial#{grp_id}"),
         InlineKeyboardButton("🖇️ ꜱʜᴏʀᴛʟɪɴᴋs", callback_data=f"set_page#shortlink#{grp_id}")],
...
def _verification_gap_text(settings):
    ...
    return (
        "<b>ʜᴇʀᴇ ʏᴏᴜ ᴄᴀɴ ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴘʀᴏᴄᴇꜱꜱ, ᴍᴇᴀɴꜱ ʏᴏᴜ ᴄᴀɴ ᴛᴜʀɴ ᴏɴ/ᴏꜰꜰ & ꜱᴇᴛ ᴛɪᴍᴇ ꜰᴏʀ 1ꜱᴛ ᴀɴᴅ 2ɴᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴘʀᴏᴄᴇꜱꜱ ᴀɴᴅ ᴀʟꜱᴏ ʏᴏᴜ ᴄᴀɴ ꜱᴇᴛ ʟᴏɢ ᴄʜᴀɴɴᴇʟ.</b>\n\n"
...
def _delete_menu_text(settings):
    ...
    for number, domain_key, api_key in ((1, "shortner", "api"), (2, "shortner_two", "api_two"), (3, "shortner_three", "api_three")):
        if settings.get(domain_key) and settings.get(api_key):
            rows.append(f"{number}ʀᴅ" if number == 3 else f"{number}ꜱᴛ" if number == 1 else f"{number}ɴᴅ")
    body = "\n".join(rows) if rows else "ɴᴏ ꜱʜᴏʀᴛᴇɴᴇʀ ɪꜱ ᴄᴜʀʀᴇɴᴛʟʏ ꜱᴇᴛ."
    return f"<b>ᴡʜɪᴄʜ ꜱʜᴏʀᴛɴᴇʀ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅᴇʟᴇᴛᴇ?</b>\n\n{body}"


def _delete_menu_buttons(settings, grp_id):
    rows = []
    available = [n for n, d, a in ((1, "shortner", "api"), (2, "shortner_two", "api_two"), (3, "shortner_three", "api_three")) if settings.get(d) and settings.get(a)]
    rows.append([InlineKeyboardButton("ꜰɪʀꜱᴛ", callback_data=f"set_delete_shortner#1#{grp_id}"), InlineKeyboardButton("ꜱᴇᴄᴏɴᴅ", callback_data=f"set_delete_shortner#2#{grp_id}"), InlineKeyboardButton("ᴛʜɪʀᴅ", callback_data=f"set_delete_shortner#3#{grp_id}")])
    rows.append([InlineKeyboardButton("ᴀʟʟ", callback_data=f"set_delete_shortner#all#{grp_id}")])
    rows.append(_back(grp_id, "shortlink_list"))
    return rows
...
def _time_page_text(settings, number):
    if number == 1:
        ...
        return (
            "<b>ʜᴇʀᴇ ʏᴏᴜ ᴄᴀɴ ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ 1ꜱᴛ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ, ᴍᴇᴀɴꜱ ᴡʜᴇɴ ꜱʜᴏᴜʟᴅ ꜱᴇᴄᴏɴᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴄᴏᴍᴇ...</b>\n\n"
            f"2ɴᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ - <code>{get_readable_time(gap)}</code>"
        )
    gap = settings.get("third_verify_time", THREE_VERIFY_GAP)
    return (
        "<b>ʜᴇʀᴇ ʏᴏᴜ ᴄᴀɴ ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ 2ɴᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ, ᴍᴇᴀɴꜱ ᴡʜᴇɴ ꜱʜᴏᴜʟᴅ ᴛʜɪʀᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴄᴏᴍᴇ...</b>\n\n"
        f"3ʀᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ - <code>{get_readable_time(gap)}</code>"
    )


def _time_page_buttons(grp_id, number):
    return [[InlineKeyboardButton("ꜱᴇᴛ ᴛɪᴍᴇ", callback_data=f"set_gap_input#{number}#{grp_id}")], _back(grp_id, "verification_gap")]
...
def _page_buttons(key, settings, grp_id):
    ...
    if key in {"auto_filter", "file_secure", "spell_check", "auto_delete", "link", "file_mode"}:
        ...
        b = _shortlink_master_buttons(settings, grp_id)
        return b
        ...
        number = settings.get("_shortener_number", 1)
        return _shortener_settings_buttons(grp_id, number)
    elif key == "delete_menu":
        return _delete_menu_buttons(settings, grp_id)
    elif key == "time_page":
        number = settings.get("_time_number", 1)
        return _time_page_buttons(grp_id, number)
    elif key == "imdb":
        b = [[InlineKeyboardButton("Set template", callback_data=f"set_input#template#{grp_id}"), InlineKeyboardButton("Default template", callback_data=f"set_default#template#{grp_id}")], [InlineKeyboardButton("Off poster", callback_data=f"set_toggle#imdb#{grp_id}")]]
    elif key == "caption":
...
def _prompt_state(query, gid, kind, origin_page, **extra):
    state = {"type": kind, "origin_page": origin_page, "prompt_chat_id": query.message.chat.id, "prompt_message_id": query.message.id}
    state.update(extra)
...
async def _edit_prompt(client, state, text, markup=None):
    try:
        return await client.edit_message_text(state["prompt_chat_id"], state["prompt_message_id"], text, reply_markup=markup, parse_mode=enums.ParseMode.HTML)
    except Exception:
        return None
...
async def settings_callback(client, query):
    ...
    try:
        if data.startswith("set_group#"):
            ...
            if not await _authorize(client, query, gid): return
        ...
        if data.startswith("advanced_settings"):
            ...
            if gid is None: return await show_group_list(client, query)
            if not await _authorize(client, query, gid): return
            return await show_group_settings(client, query, gid)
        ...
        if action == "set_back":
            ...
            if key == "main": return await show_group_settings(client, query, gid)
            return await show_page(client, query, key, gid)
        if action == "set_close":
            PENDING.pop((query.from_user.id, gid), None)
        ...
        if action == "set_delete":
            if key == "request_channel": await save_group_settings(gid, key, int(REQUEST_CHANNEL))
            return await show_page(client, query, key, gid)
        ...
        if action == "set_shortner_action":
            ...
            if mode == "set":
                ...
                return await _edit_prompt(client, state, "<b>ꜱᴇɴᴅ ᴍᴇ ꜱʜᴏʀᴛʟɪɴᴋ ᴜʀʟ ᴡɪᴛʜᴏᴜᴛ https ꜰᴏʀᴍᴀᴛ -</b>\n\n<code>https://tnshort.net</code> ❌\n<code>tnshort.net</code> ✅\n\n<b>/cancel</b> - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.")
            # remove one shortener
            domain_key = {1: "shortner", 2: "shortner_two", 3: "shortner_three"}[number]
            api_key = {1: "api", 2: "api_two", 3: "api_three"}[number]
        ...
        if action == "set_delete_shortner":
            ...
            if not settings.get(domain_key) or not settings.get(api_key):
                return await query.message.edit_text("<b>💔 ꜰɪʀꜱᴛ ᴀᴅᴅ ᴛʜɪs ꜱʜᴏʀᴛᴇɴᴇʀ ꜰᴏʀ ᴅᴇʟᴇᴛɪᴏɴ.</b>", reply_markup=InlineKeyboardMarkup([_back(gid, "delete_menu")]), parse_mode=enums.ParseMode.HTML)
            await save_group_settings(gid, domain_key, "")
            await save_group_settings(gid, api_key, "")
            return await query.message.edit_text(f"<b>ᴅᴇʟᴇᴛᴇ {number} ꜱʜᴏʀᴛʟɪɴᴋ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅</b>", reply_markup=InlineKeyboardMarkup([_back(gid, "delete_menu")]), parse_mode=enums.ParseMode.HTML)
        if action == "set_gap":
        ...
        if action == "set_gap_input":
            ...
            return await _edit_prompt(client, state, "<b>ꜱᴇɴᴅ ᴍᴇ ᴀ ᴛɪᴍᴇ ɪɴ ʟɪᴋᴇ ᴛʜɪs -</b> <code>1h</code> ᴏʀ <code>15m</code>\n\n<b>/cancel</b> - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.")
    except Exception as exc:
        try: await query.answer("💔 ꜱᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ...", show_alert=True)
...
async def advanced_cancel(client, message):
    ...
    # Return to exactly the page where the input flow started.
    page = state.get("origin_page", "shortlink_list")
    try:
        if page == "shortlink_list":
            settings = await get_settings(gid)
            await client.edit_message_text(state["prompt_chat_id"], state["prompt_message_id"], _shortlink_list_text(settings), reply_markup=InlineKeyboardMarkup(_shortlink_list_buttons(gid)), parse_mode=enums.ParseMode.HTML)
        elif page == "verification_gap":
            settings = await get_settings(gid)
            await client.edit_message_text(state["prompt_chat_id"], state["prompt_message_id"], _verification_gap_text(settings), reply_markup=InlineKeyboardMarkup(_verification_gap_buttons(gid)), parse_mode=enums.ParseMode.HTML)
        else:
...
async def advanced_input(client, message):
    ...
    if not uid: raise ContinuePropagation
    candidates = [(k, v) for k, v in PENDING.items() if k[0] == uid]
    if not candidates: raise ContinuePropagation
    ...
    if not value: return await message.reply_text("ᴠᴀʟᴜᴇ ᴄᴀɴɴᴏᴛ ʙᴇ ᴇᴍᴘᴛʏ")
    key = state["type"]

    # New three-shortener UI flow: domain -> API -> live verification -> success.
    if key.startswith("shortner_") and key.endswith("_domain"):
        ...
        if not re.fullmatch(r"[A-Za-z0-9.-]+(?::\d+)?", domain):
            return await _edit_prompt(client, state, "<b>❌ ɪɴᴠᴀʟɪᴅ ꜱʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ.</b>\n\nꜱᴇɴᴅ ᴏɴʟʏ ᴛʜᴇ ᴅᴏᴍᴀɪɴ, ᴇxᴀᴍᴘʟᴇ: <code>tnshort.net</code>\n\n<b>/cancel</b> - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.")
        state["type"] = f"shortner_{number}_api"
        state["domain"] = domain
        PENDING[(uid, gid)] = state
        return await _edit_prompt(client, state, "<b>sᴇɴᴅ ᴍᴇ ᴀ sʜᴏʀᴛʟɪɴᴋ ᴀᴘɪ...</b>\n\n<b>/cancel</b> - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.")
    ...
    if key.startswith("shortner_") and key.endswith("_api"):
        ...
        PENDING.pop((uid, gid), None)
        return await _edit_prompt(client, state, f"<b>ᴛʜᴇ {number}ꜱᴛ/ɴᴅ/ʀᴅ ꜱʜᴏʀᴛʟɪɴᴋ ᴡᴀꜱ ᴀᴅᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅</b>" if number == 1 else f"<b>ᴛʜᴇ {number}ɴᴅ/ʀᴅ ꜱʜᴏʀᴛʟɪɴᴋ ᴡᴀꜱ ᴀᴅᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅</b>", InlineKeyboardMarkup([_back(gid, "shortlink_list")]))

    if key.startswith("gap_"):
        number = state["number"]
        seconds = _parse_duration(value)
        if seconds is None:
            return await _edit_prompt(client, state, "<b>❌ ᴘʟᴇᴀsᴇ sᴇɴᴅ ᴀ ᴠᴀʟɪᴅ ᴛɪᴍᴇ ʟɪᴋᴇ <code>1h</code> ᴏʀ <code>15m</code>.</b>\n\n<b>/cancel</b> - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.")
        setting_key = "verify_time" if number == 1 else "third_verify_time"
        await save_group_settings(gid, setting_key, seconds)
        PENDING.pop((uid, gid), None)
        return await _edit_prompt(client, state, f"<b>ᴛɪᴍᴇ {number} sᴇᴛ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅</b>", InlineKeyboardMarkup([_back(gid, "verification_gap")]))

    # Keep the existing advanced settings inputs for all other settings.
    if key == "delete_time":
        try: value_int = int(value); assert value_int > 0
        except Exception: return await message.reply_text("Send a valid positive number of seconds or /cancel")
        await save_group_settings(gid, "delete_time", value_int); PENDING.pop((uid, gid), None)
    elif key == "max_results":
        try: value_int = int(value); assert 1 <= value_int <= 20
        except Exception: return await message.reply_text("Max results must be between 1 and 20.")
        await save_group_settings(gid, "max_results", value_int); PENDING.pop((uid, gid), None)
    elif key == "template":
        if any(x not in value for x in ("{search}", "{mention}", "{group}")): return await message.reply_text("Template must support {search}, {mention}, and {group}, or use /cancel.")
        await save_group_settings(gid, "template", value); PENDING.pop((uid, gid), None)
    elif key == "caption":
        if "{file_name}" not in value: return await message.reply_text("Caption must contain {file_name}, or use /cancel.")
        await save_group_settings(gid, "caption", value); PENDING.pop((uid, gid), None)
    elif key.startswith("tutorial"):
        if not (value.startswith("http://") or value.startswith("https://")): return await message.reply_text("Send a valid http/https URL or /cancel")
        await save_group_settings(gid, key, value); PENDING.pop((uid, gid), None)
    elif key in {"shortner", "shortner_two", "shortner_three", "api", "api_two", "api_three", "verify_time", "third_verify_time"}:
        # Legacy callbacks are retained for compatibility; the new UI uses the stateful flow above.
        await save_group_settings(gid, key, value); PENDING.pop((uid, gid), None)
    elif key == "request_channel":
        try: value_int = int(value)
        except ValueError: return await message.reply_text("Send a valid channel ID or /cancel")
        await save_group_settings(gid, key, value_int); PENDING.pop((uid, gid), None)
    elif key == "fsub_add":
        try: value_int = int(value)
        except ValueError: return await message.reply_text("Send a valid channel ID or /cancel")
        settings = await get_settings(gid); channels = list(settings.get("fsub_channels") or [])
        if value_int not in channels: channels.append(value_int)
        await save_group_settings(gid, "fsub_channels", channels); PENDING.pop((uid, gid), None)
    elif key == "fsub_delete":
        try: value_int = int(value)
        except ValueError: return await message.reply_text("Send a valid channel ID or /cancel")
        settings = await get_settings(gid); channels = [int(c) for c in (settings.get("fsub_channels") or []) if int(c) != value_int]
        if not channels: channels = [AUTH_CHANNEL]
        await save_group_settings(gid, "fsub_channels", channels); PENDING.pop((uid, gid), None)
    else:
        await save_group_settings(gid, key, value); PENDING.pop((uid, gid), None)
