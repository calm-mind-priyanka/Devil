...
async def pm_search(client, message):
    ...
    if await db.get_pm_search_status(bot_id):
        try:
            ...
            logger.exception("PM movie search failed")
            return await message.reply_text(
                "❌ <b>ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ᴇʀʀᴏʀ.</b>\n<code>{}</code>".format(str(e)[:500]),
                quote=True,
            )
    else:
    ...
    if settings["auto_filter"]:
        ...
        if (
            ...
            try:
                ...
                traceback.print_exc()
                logger.exception("Group movie search failed")
                try:
                    await message.reply_text(
                        "❌ <b>ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ᴇʀʀᴏʀ.</b>\n<code>{}</code>".format(str(e)[:500]),
                        quote=True,
...
async def next_page(bot, query):
    ...
    links = ""
    if settings.get("link", LINK_MODE):
        btn = []
...
async def cb_handler(client: Client, query: CallbackQuery):
    data = query.data or ""
    # Canonical settings callbacks live in advanced_settings.py. This legacy
    # catch-all must not consume them before that handler receives the query.
    if (data == "settings_groups" or data.startswith(("settings_group#", "grp_setting#", "setting_cancel#", "file_mode#", "advanced_settings", "adv_"))):
...
async def auto_filter(client, msg, spoll=False, pm_mode=False):
    if not spoll:
        ...
        searching_msg = await msg.reply_text(f"🔎 sᴇᴀʀᴄʜɪɴɢ <code>{search}</code>")
        max_results = int(settings.get("max_results", MAX_BTN))
        try:
            files, offset, total_results = await get_search_results(search, max_results=max_results)
        except Exception as e:
            await searching_msg.edit_text("❌ <b>ᴅᴀᴛᴀʙᴀꜱᴇ ꜱᴇᴀʀᴄ ᴇʀʀᴏʀ.</b>\n<code>{}</code>".format(str(e)[:500]))
            logger.exception("Database search failed")
            return
        await searching_msg.delete()
        if not files:
            if settings.get("spell_check", SPELL_CHECK):
            ...
            k = await msg.reply_text(script.NO_RESULT_TXT, quote=True)
            await asyncio.sleep(min(delete_time, 60))
            try:
                await k.delete()
            except Exception:
                pass
